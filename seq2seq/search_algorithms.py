...
def multimodal_beam_search(
    ...
    """
    Implements Beam Search to compute the output with the sequences given in X_smiles and in X_descriptors. The method can compute 
    several outputs in parallel.

    Parameters
    ----------    
    X_smiles: LongTensor of shape (examples, length)
        The sequences to start the decoding process.

    X_descriptors: LongTensor of shape (examples, descriptors_length)
        The molecular descriptors of the sequences to start the decoding process.
    ...
    """
    with torch.no_grad():
        Y = torch.ones(X_smiles.shape[0], 1).to(next(model.parameters()).device).long()
        # The next command can be a memory bottleneck, can be controlled with the batch 
        # size of the predict method.
        next_probabilities = model.forward(X_smiles, X_descriptors, Y)[:, -1, :]
        vocabulary_size = next_probabilities.shape[-1]
        probabilities, next_chars = next_probabilities.squeeze().log_softmax(-1)\
        .topk(k = beam_width, axis = -1)
        Y = Y.repeat((beam_width, 1))
        ...
        if progress_bar > 0:
            predictions_iterator = tqdm(predictions_iterator)
        for i in predictions_iterator:
            dataset = tud.TensorDataset(X_smiles.repeat((beam_width, 1, 1)).transpose(0, 1).flatten(end_dim = 1), X_descriptors.repeat((beam_width, 1, 1)).transpose(0, 1).flatten(end_dim = 1), Y)
            loader = tud.DataLoader(dataset, batch_size = batch_size)
            next_probabilities = []
            iterator = iter(loader)
            if progress_bar > 1:
                iterator = tqdm(iterator)
            for x_smiles, x_descriptors, y in iterator:
                next_probabilities.append(model.forward(x_smiles, x_descriptors, y)[:, -1, :].log_softmax(-1))
            next_probabilities = torch.cat(next_probabilities, axis = 0)
            next_probabilities = next_probabilities.reshape((-1, beam_width, next_probabilities.shape[-1]))
            probabilities = probabilities.unsqueeze(-1) + next_probabilities
            probabilities = probabilities.flatten(start_dim = 1)
            ...
            next_chars = torch.remainder(idx, vocabulary_size).flatten().unsqueeze(-1)
            best_candidates = (idx / vocabulary_size).long()
            best_candidates += torch.arange(Y.shape[0] // beam_width, device = X_smiles.device).unsqueeze(-1) * beam_width
            Y = Y[best_candidates].flatten(end_dim = -2)
            Y = torch.cat((Y, next_chars), axis = 1)
        return Y.reshape(-1, beam_width, Y.shape[-1]), probabilities